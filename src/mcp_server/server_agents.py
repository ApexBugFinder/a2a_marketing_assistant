...
_raw_embedding_model = os.getenv("GEMINI_EMBEDDING_MODEL", "gemini-embedding-001")
# Strip LiteLLM provider prefix (e.g. "gemini/gemini-embedding-2" → "gemini-embedding-2")
EMBEDDING_MODEL = _raw_embedding_model.split('/')[-1] if '/' in _raw_embedding_model else _raw_embedding_model
     ...
     try:
          if agent_cards:
               ...
               for i, card in enumerate(agent_cards):
                    if i > 0:
                         time.sleep(1)
                    embeddings.append(generate_embeddings(json.dumps(card)))
...
def serve (host, port, transport):
     ...
     mcp = FastMCP('Marketing Assistant Server Agent Cards MCP Server')
...
if __name__ == "__main__":
     serve(host='0.0.0.0', port=10100, transport='streamable-http')