...
class WorkflowNode:
     ...
     async def get_planner_resource(self) -> AgentCard | None:
          """Fetches the planner resource (e.g., agent card) associated with this node."""
     ...
     async def find_agent_for_tasks(self) -> AgentCard | None:
          """Finds the appropriate agent to execute the task."""
          ...
          async with client.init_session(
               ...
               data = json.loads(response.contents[0].text)
               logger.debug(f"Found agent {data} for task: \n {self.task}\n\n")
               agent_card = AgentCard(**data['agent_card'][0])
               return agent_card
     ...
     async def run_node(self, query: str, task_id: str, context_id: str) ->  AsyncIterable[dict[str, any]]:
          ...
          if self.node_key == 'planner':
               ...
               agent_card = await self.find_agent_for_tasks()
          async with httpx.AsyncClient() as httpx_client:
               a2a_client = A2AClient(http_client=httpx_client, agent_card=agent_card)
               ...
               request = SendStreamingMessageSuccessResponse(id=str(uuid()),
                                                            params=MessageSendParams(**payload))
               response_stream = a2a_client.send_message_streaming(request)
               async for chunk in response_stream:
                    logger.debug(f"Received chunk from agent {agent_card.name} for node {self.id}: \n{chunk}\n\n")
                    # Save teh artifact as a result of the node
               ...
               async for chunk in node.run_node(query, task_id, context_id):
                    ...
                    if node.state != Status.PAUSED:
                         if isinstance(
                              ...
                              if (
                                   task_status_event.status ==TaskState.TASK_STATE_INPUT_REQUIRED
                                   and context_id
                              ):
